...
export const rm = async (fsObject, recursive, limitToCWDFilesystem, onlyIfExists) => {
  ...
  if (rmSh.error) {
    let msg
    if (rmSh.output.includes('Directory not empty')) msg = `${LOCAL.directoryNotEmpty}: rm: ${fsObject}`
    else if (rmSh.output.includes('Permission denied')) msg = `${LOCAL.permissionDenied}: rm: ${fsObject}`
    else if (rmSh.output.includes('No such file or directory')) msg = `${LOCAL.noFileOnRM}: rm: ${fsObject}`
