...
describe('chown', () => {
  ...
  step('can read FS Objects user and group', async () => {
    ...
    const chownD1 = await tstDir.addDirectory('chownD1', true)
    expect(`${await chownD1.user}`).to.equal(process.env.USER)
  ...
  step('can set FS Objects user and group', async () => {
    ...
    const P1 = u(`${tstDir}/chownD2A`, rootShell)
    const P2 = await P1.u('P2')
    await expect(P1.setUser('testusr', 'testusr')).to.be.fulfilled
    expect(P1.state).to.equal(`loadable`)
    await P1.stat()
  ...
  step('can set FS Objects user only (also recursively)', async () => {
    ...
    const P1 = u(`${tstDir}/chownF`, rootShell)
    const P2 = await P1.u('P2')
  ...
  step('can set FS Objects group only (also recursively)', async () => {
    ...
    const P1 = u(`${tstDir}/chownG`, rootShell)
    const P2 = await P1.u('P2')
